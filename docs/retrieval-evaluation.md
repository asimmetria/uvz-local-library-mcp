...
Скрипт выбирает разные aliases внутренних библиотек, для которых индекс нашёл
владельца и consumer с известной configuration, затем фиксирует одного consumer
для каждого и ставит `review_required: true`. Внешние зависимости Spring и
драйверы намеренно не попадают в черновик; для их отдельной проверки существует
флаг `--include-external`. Скрипт не перезаписывает
