...
Никогда не вызывай `next` или `start`, пока текущий repository имеет статус
`running`. Controller отклонит такой вызов: одновременно разрешён ровно один
активный repository.
...
Финальный campaign report детерминированно выводит не только totals, но и
`failed_repositories` с именем, путём, числом попыток и последней причиной.
...
- Не пропускай repository и не ставь `failed` только потому, что он большой,
  ...
  создай и проверь карточки. `failed` допустим только при конкретной ошибке
  записи, безопасности, tool/runtime или невозможности подтвердить обязательный
  schema-контракт; укажи точную ошибку в `message`.
